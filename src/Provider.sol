...
abstract contract Provider is User {
     ...
     *
     * @dev Emits `ProtocolAnnounced(protocol, name, description)` on success.
     ...
     *      - `Forbidden` if context hash mismatch
     *      - `InsufficientFunding` if amount exceeds declared funding
    ...
    external  nonReentrant( LOCK_TRANSFER_FUNDING )  returns ( uint256 updated_index, uint256 new_available_amount )
    {
        ...
        uint256 current_context_hash     =  _read_smart_var( SLOT_CURRENT_CONTEXT_HASH );
        if(  calculated_context_hash != current_context_hash  )  revert Forbidden( msg.sender, calculated_context_hash, current_context_hash );

        // ━━━━  STEP 2: Find the funding entry for this token  ━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━

        uint256 index  =  _find_funding_index( context.fundings, token );
        if(  index == INDEX_NOT_FOUND  )  revert InsufficientFunding( address(token), 0, amount );

        uint256 declared_available  =  context.fundings[ index ].amount;
        ...
        uint256 held_from_stake         =  ( stake_matches_token )  ?  _read_smart_var( SLOT_HELD_STAKE )  :  0;
        uint256 held_from_msg_value     =  ( is_native_token )  ?  _read_smart_var( SLOT_HELD_MSG_VALUE )  :  0;
        ...
        //  *GAS SAVING*  -  Only write slots that are relevant to this token.
        if(  stake_matches_token  )  _write_smart_var( SLOT_HELD_STAKE, held_from_stake );
        if(  is_native_token  )      _write_smart_var( SLOT_HELD_MSG_VALUE, held_from_msg_value );
        ...
        uint256 new_context_hash  =  HashLib.calc_context_hash( IBondRouteProtected(msg.sender), context );
        _write_smart_var( SLOT_CURRENT_CONTEXT_HASH, new_context_hash );
    }
    ...
    /**
     * @notice Transfer using held funds first, then pull remainder from user
     * @param token Token to transfer
     * @param from User address to pull funds from if held funds insufficient
     * @param to Destination address for the transfer
     * @param amount Total amount to transfer
     * @param held_from_stake Amount held from stake
     * @param held_from_msg_value Amount held from msg.value
     * @return new_held_from_stake Updated amount held from stake
     * @return new_held_from_msg_value Updated amount held from msg.value
     * @dev Consumes from held funds first (stake, then msg.value), pulls remainder from user
     */
    ...
    private returns ( uint256 new_held_from_stake, uint256 new_held_from_msg_value )
    {
        ...
        uint256 amount_from_held  =  _min( amount, total_held );
        uint256 amount_to_pull;
        ...
        if(  amount_from_held > 0  )
        {
            ...
            uint256 from_stake  =  _min( amount_from_held, held_from_stake );
            uint256 from_msg_value;
    ...
    /**
     * @notice Find index of funding entry for a given token
     * @param fundings Array of fundings
     * @param token Token to find
     * @return index Index of the funding entry, or type(uint256).max if not found
     */
    function _find_funding_index( TokenAmount[] memory fundings, IERC20 token )
