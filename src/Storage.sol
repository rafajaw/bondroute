...
abstract contract Storage is SmartReentrancyGuard {
    ...
    address internal _pending_sweeper;

    // Accumulated tips per token, claimable by liquidator via `claim_accumulated_tips()`.
    // All tips are accumulated here regardless of source (held funds or pulled from user).
    // Key = token (`IERC20(address(0))` for native). Value = accumulated tip amount.
    mapping( IERC20 => uint256 ) internal _accumulated_tips;

    constructor( address eip1153_detector )
    SmartReentrancyGuard( eip1153_detector ) { }

    function _create_bond_internal( bytes32 commitment_hash, TokenAmount memory stake, uint256 amount_received )
    internal
    {
     ...
     */
    function _set_bond_status( bytes32 bond_key, uint256 previous_packed_value, BondStatus new_status )
    internal
    {
        _bonds[ bond_key ]  =  previous_packed_value | ( uint256(uint8(new_status)) << 248 );
    }

    function _read_smart_var( uint256 slot )
    ...
    function _write_smart_var( uint256 slot, uint256 value )
    internal
    {
        bool use_transient  =  _has_transient_storage_support( );
