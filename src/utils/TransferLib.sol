...
library TransferLib {
     ...
     */
    function transfer_native( address to, uint256 amount )
    internal
    {
        if(  amount == 0  )  return;
     ...
     */
    function transfer_erc20( IERC20 token, address from, address to, uint256 amount )
    internal
    {
        if(  amount == 0  )  return;
     ...
     * @dev Reverts with `TransferFailed` if the token transfer fails.
     */
    function transfer_erc20_and_get_amount_delivered( address from, IERC20 token, uint256 amount, address to )
    internal returns ( uint256 amount_delivered )
    {
        if(  amount == 0  )  return 0;

        uint256 balance_before  =  token.balanceOf( to );
        ...
        uint256 balance_after  =  token.balanceOf( to );
     ...
     */
    function transfer( IERC20 token, address from, address to, uint256 amount )
    internal
    {
