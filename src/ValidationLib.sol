...
import { Reentrancy } from "./utils/SmartReentrancyGuard.sol";
...
library ValidationLib {
    ...
    function is_valid_execution( ExecutionData memory execution_data )
    internal view returns ( bool is_valid, string memory invalid_reason )
    {
        // *GAS SAVING*  -  Since we are checking BondRoute support we can skip checks on the called contract's address
        //                  and its code length as that would only increase code complexity and gas costs.
    ...
    function is_valid_fundings( TokenAmount[] memory fundings )
    internal pure returns ( bool is_valid, string memory invalid_reason )
    {
    ...
    function _does_support_bondroute_and_selector( IBondRouteProtected target_contract, bytes4 target_selector )
    ...
    function revert_if_possibly_bond_farming( bytes memory call_output )
    internal pure
    {
     ...
     */
    function validate_typed_string_prefix( string memory typed_string )
    internal pure
    {
     ...
     */
    function validate_TokenAmount_definition( string memory typed_string, uint256 TokenAmount_offset )
    internal pure
    {
        ...
        bytes memory typed_bytes  =  bytes(typed_string);
        uint256 min_length  =  TokenAmount_offset + 41;  // "TokenAmount(address token,uint256 amount)" is 41 chars.
