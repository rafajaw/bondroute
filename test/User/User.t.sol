...
import { User, PastCreationDeadline, NativeAmountMismatch, InvalidSignature, EIP712Domain } from "@BondRoute/User.sol";
...
contract UserHarness is User {
    ...
    constructor( address eip1153_detector ) User( eip1153_detector ) {}
...
contract UserTest is Test {
    ...
    function setUp() public
    {
        ...
        user_harness      =  new UserHarness( address(eip1153_detector) );
    ...
    function test_create_bond_with_erc20_stake() public
    {
        ...
        vm.prank( USER );
        user_harness.create_bond( commitment_hash, stake, deadline );
    ...
    function test_create_bond_with_native_stake() public
    {
        ...
        vm.prank( USER );
        user_harness.create_bond{ value: 1 ether }( commitment_hash, stake, deadline );
    ...
    function test_create_bond_reverts_on_zero_commitment_hash() public
    {
        ...
        vm.prank( USER );
        user_harness.create_bond( bytes32(0), stake, deadline );
    }

    function test_create_bond_reverts_on_past_deadline() public
    {
        TokenAmount memory stake  =  TokenAmount({ token: usdc, amount: 100e6 });
        ExecutionData memory execution_data  =  _create_test_execution_data( stake, 7 );

        bytes32 commitment_hash  =  user_harness.__OFF_CHAIN__calc_commitment_hash( USER, execution_data );
        uint256 past_deadline  =  block.timestamp - 1;

        vm.expectRevert( abi.encodeWithSelector( PastCreationDeadline.selector, commitment_hash, past_deadline ) );

        vm.prank( USER );
        user_harness.create_bond( commitment_hash, stake, past_deadline );
    }
    ...
    function test_create_bond_reverts_on_native_amount_mismatch() public
    {
        ...
        vm.prank( USER );
        user_harness.create_bond{ value: 0.5 ether }( commitment_hash, stake, deadline );
    }
    ...
    function test_create_bond_reverts_on_reentrancy() public
    {
        ...
        bytes memory reentrancy_call  =  abi.encodeCall(
            ...
            ( commitment_hash, stake, deadline )
        );
