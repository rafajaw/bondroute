...
contract BondRouteTest is Test {
    ...
    function _create_bond_from_execution_with_value( address bond_creator, address user, ExecutionData memory execution_data, uint256 value ) internal returns ( bytes32 commitment_hash )
    {
        ...
        vm.prank( bond_creator );
        bond_route.create_bond{ value: value }( commitment_hash, execution_data.stake, block.timestamp + 1 hours );
    }
    ...
    {
        BondRoute new_bond_route  =  new BondRoute( SWEEPER, address(eip1153_detector) );
    ...
    function test_constructor_reverts_on_zero_sweeper() public
    {
        ...
        new BondRoute( address(0), address(eip1153_detector) );
    }

    function test_constructor_reverts_on_bad_eip1153_detector() public
    {
        vm.expectRevert();
        new BondRoute( SWEEPER, address(0x1234) );
    }
    ...
    function test_receive_reverts() public
    {
        ...
        assertEq( string(return_data), string(abi.encodeWithSignature("Error(string)", "Use tip() to donate")), "Should revert with correct message" );
    }
    ...
    function test_integration_create_and_execute_with_tip() public
    {
        vm.prank( USER );
        bond_route.tip( usdc, 100e6, "Thanks for the MEV protection!" );

        uint256 claimable  =  bond_route.get_claimable_tips( usdc );
        assertEq( claimable, 100e6 - 1, "Tips should be claimable (minus dust)" );
    }
    ...
    function test_integration_create_expire_liquidate() public
    {
        ...
        uint256 sweeper_balance_before  =  usdc.balanceOf( SWEEPER );

        vm.prank( SWEEPER );
        bond_route.liquidate_expired_bonds( commitment_hashes, stakes, SWEEPER );

        uint256 sweeper_balance_after  =  usdc.balanceOf( SWEEPER );
        assertEq( sweeper_balance_after, sweeper_balance_before + 100e6, "Sweeper should receive liquidated stake" );
    }
