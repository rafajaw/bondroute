...
contract InvariantHandler is Test {
    ...
    function create_bond( uint256 actor_seed, uint256 token_seed, uint256 amount_seed, uint256 with_funding_seed ) external {
        ...
        vm.prank( actor );
        try bondRoute.create_bond{ value: token == IERC20(address(0)) ? amount : 0 }( commitment_hash, stake, deadline )
        {
    ...
    function liquidate_bond( uint256 bond_seed ) external {
        ...
        address sweeper  =  address(this);  // Handler acts as sweeper

        try bondRoute.liquidate_expired_bonds( commitment_hashes, stakes, sweeper ) {
